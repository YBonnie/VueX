<template>
	<div class="hotmusic animated fadeIn">
		<p class="tip">歌曲列表 (共{{count}}首)</p>
		<div class="songlist">
			<van-row class="listBox"  v-for="(item,index) in songs" :key="index" :ref="'van' + index" @click.native="goPath(item)">
				...
				<van-col span="16">
					<van-row>
						...
						<van-col span="24" class="songAuthor"><i class="iconfont icon-hot"></i>{{item.singer}}</van-col>
					</van-row>
				</van-col>
...
<script>
	...
	export default{
		...
		created(){
			...
			this.id = this.$store.state.playlists[30].id
			this.count = this.$store.state.playlists[30].songNum
